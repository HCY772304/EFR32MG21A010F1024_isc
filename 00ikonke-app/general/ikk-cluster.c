...
#define ENDPOINT_MAXN				48
#define CLUSTER_MAXN				6
...
//						}
    ...
    case (ZCL_ON_OFF_CLUSTER_ID): {
        ...
        // command c->s
        if ((cmd->clusterSpecific) && (cmd->direction == (uint8_t)ZCL_DIRECTION_CLIENT_TO_SERVER) && (!cmd->mfgSpecific)) {
            switch (cmd->commandId) {
        ...
        } else if ((!cmd->clusterSpecific) && (cmd->direction == (uint8_t)ZCL_DIRECTION_SERVER_TO_CLIENT)
            && (!cmd->mfgSpecific)) {
            uint16_t attribute_id = U16MERG(cmd->buffer[cmd->payloadStartIndex + 1], cmd->buffer[cmd->payloadStartIndex + 0]);
            uint8_t dataType = cmd->buffer[cmd->payloadStartIndex + 2];
            uint8_t generalCommandFrame = cmd->buffer[cmd->payloadStartIndex - 1];
            iKonkeAfSelfPrint("####ON OFF Cluster Report attrId(%02x) seq(%x) payloadStartIndex(%d) generalCommandFrame(0x%x)dataType(%d)epEnable(%d)Payload:\r\n", attribute_id, cmd->seqNum, cmd->payloadStartIndex, generalCommandFrame, dataType, emberAfEndpointIsEnabled(cmd->apsFrame->destinationEndpoint));
            iKonkeAfSelfPrintBuffer(cmd->buffer, cmd->bufLen, true);
            iKonkeAfSelfPrint("\r\n");

            if (generalCommandFrame == ZCL_REPORT_ATTRIBUTES_COMMAND_ID && attribute_id == ZCL_ON_OFF_ATTRIBUTE_ID && dataType == ZCL_BOOLEAN_ATTRIBUTE_TYPE && emberAfEndpointIsEnabled(cmd->apsFrame->destinationEndpoint)) {
                if (sender != 0x0000) {
                    uint8_t index = 0;
                    uint8_t storedSequence = kSequenceFindByNodeidAndEp(sender, cmd->apsFrame->destinationEndpoint, &index);

                    OnOffStatusEnum status = kZclOnOffClusterServerOnOffGet(cmd->apsFrame->destinationEndpoint);
                    iKonkeAfSelfPrint("##### LinkageExecute normal sender_node(%02x) index(%d) localep(%d) storedseq(0x%x) senderseq(0x%x) commandId(%d) localonoffstatus(%d)\r\n",
                        sender, index, cmd->apsFrame->destinationEndpoint, storedSequence, cmd->seqNum, cmd->commandId, status);
                    if (cmd->seqNum <= storedSequence && storedSequence - cmd->seqNum <= 60) {
                        iKonkeAfSelfPrint("#####OnOff LinkageExecute sequence too early, sender_node(%02x) index(%d) localep(%d) storedseq(0x%x) senderseq(0x%x) command(%d)\r\n",
                            sender, index, cmd->apsFrame->destinationEndpoint, storedSequence, cmd->seqNum, cmd->commandId);
                        // add by maozj 20191224
                        //                if (g_pfSceneClusterRecallCallback){
                        //                  g_pfSceneClusterRecallCallback(cmd->apsFrame->destinationEndpoint, cmd->apsFrame->clusterId, cmd->commandId, false, false);//restore immediately report
                        //                }
                        return true;
                    } else {
                        kSequenceAdd(sender, cmd->apsFrame->destinationEndpoint, cmd->seqNum);
                        // add by maozj 20191224
                        if (g_pfSceneClusterRecallCallback) {
                ...
                uint8_t onOffValue = cmd->buffer[cmd->payloadStartIndex + 3];
                iKonkeAfSelfPrint("Multi-ContorlWrite Attribute ep(%d) value(%d)\r\n", cmd->apsFrame->destinationEndpoint, onOffValue);
                switch (onOffValue) {
                case 0x00:
                case 0x01:
                case 0x02: {
          if(kZclOnOffClusterServerOnOffGet(cmd->apsFrame->destinationEndpoint) != onOffValue){
                    ...
                    MEMCOPY(ZclOnOffControlInfo.maskEui64Addr, senderEui64, 8);
    ...
    if (endpoint != 0xFF) {
        ...
        if (index != 0xFF) {
            ...
            if (maskEui64Addr != NULL) {
                ...
                // iKonkeAfSelfPrint("### writeMaskEui64\r\n");
                // iKonkeAfSelfPrint("###11111111, euiAddr(%x)(%x)(%x)(%x)(%x)(%x)(%x)(%x)\r\n", \
        //    (maskEui64Addr)[0], (maskEui64Addr)[1],\
        //    (maskEui64Addr)[2],  (maskEui64Addr)[3],\
        //    (maskEui64Addr)[4],  (maskEui64Addr)[5],\
        //    (maskEui64Addr)[6],  (maskEui64Addr)[7]);
